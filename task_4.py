...
def summ_of_elem(counter, num=1, sum_els=0, i=1):
    ...
    if counter > 0:  # пока счетчик не 0 считаем
        # если i (порядковый номер операции) четный, то надо вычетать из суммы
        # тек. значение элемента ряда
        if i % 2 == 0:
        ...
        else:  # если i (порядковый номер операции) нечетный, то надо
            # прибалять к сумме тек. значение элемента ряда
            sum_els = sum_els + num  # вычисляем сумму ряда
            num = num / 2  # вычисляем следующий элемент числового ряда
            counter -= 1  # изменяем счетчик цикла
            i += 1  # вычисляем порядковый номер следующей итерации
...
# --------------------------------var 2-----------------------------------
'''я пока не виже смысла использовать рекурсию где-то кроме обхода древа
каталогов, мне кажется решение через цикл проще и понятней,
хотя я наверное еще не начал думать рекурсией.
'''
try:
