...
def check_1(lst_obj):
    ...
    Сложность: !!!.
    """
    lst_to_set = set(lst_obj)  # !!! константа
    return lst_to_set
...
def check_2(lst_obj):
    ...
    Сложность: !!! Линейная.
    """
    for j in range(len(lst_obj)):          # !!! квадратичная?
...
def check_3(lst_obj):
    ...
    lst_copy.sort()                          # !!! линейная ? надо посмотреть документацию, как работает сорт
    for i in range(len(lst_obj) - 1):        # !!! линейная так как в цикле перебор все равно придется перебрать n эелемнтов
...
for j in (50, 500, 1000, 5000, 1000):
    ...
    # Всего 10 тыс. чисел
    lst = random.sample(range(-100000, 100000), j) # линейная, не важно сколько брать, всегда будет выполняться n раз
